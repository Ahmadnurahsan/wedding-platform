...
export function NoirTheme({ data, slug, lang, setLang, guestName, guestCode }: NoirThemeProps) {
  ...
  return (
    <div ref={mainRef} className="min-h-screen bg-[#0a0a0a] font-['Poppins',sans-serif] overflow-x-hidden" style={{ color: 'rgb(196,201,203)' }}>
      ...
      {/* Dynamic content background — changes every 3s */}
      ...
      <button onClick={() => setMenuOpen(true)}
        className={`fixed left-8 bottom-8 z-50 flex h-12 w-12 items-center justify-center bg-white/10 backdrop-blur-md hover:bg-white/20 transition-all rounded-lg ${scrolled ? 'flex' : 'hidden'}`}>
        <Menu className="h-5 w-5" />
      </button>

      {/* ===== HERO ===== */}
      <section id="home" ref={setSectionRef} className="relative h-screen overflow-hidden">
        {heroImages.map((img, i) => (
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10">
          <button onClick={() => scrollTo('couple')} className="flex flex-col items-center gap-2 text-white/40 hover:text-white/70 transition-all">
      ...
      <section id="couple" ref={setSectionRef} className="px-6 py-32 text-left max-w-2xl mx-auto">
        ...
        <div className="mt-20">
          {/* Tabs — plain text, no underline, left aligned */}
          ...
          <div className="flex gap-10 mt-16">
            {[
              ...
              <div key={item.l} className="flex flex-col items-start">
                <span className="text-7xl md:text-8xl font-extrabold tracking-tight text-white/70">{String(item.v).padStart(2, '0')}</span>
                <span className="text-sm uppercase tracking-[0.25em] mt-4 opacity-30 font-bold">{item.l}</span>
              </div>
            ))}
          </div>
          <button onClick={() => {
      ...
      {/* ===== EVENTS — black text on white transparent bg ===== */}
      <section id="event" ref={setSectionRef} className="px-6 py-32">
        <div className="max-w-2xl mx-auto">
          <div className="p-10 md:p-16">
            ...
            {/* Main card: Akad + Resepsi + Dress Code */}
            <div className="bg-white/10 backdrop-blur-md rounded-lg p-10 md:p-14 space-y-10">
              {['AKAD NIKAH', 'RESEPSI'].map((title, i) => (
                <div key={i}>
                  <p className="font-semibold tracking-wider opacity-90 mb-6" style={{ fontSize: '36px', lineHeight: '36px' }}>{title}</p>
                  <div className="space-y-3">
                  ...
                  <button onClick={() => window.open('https://maps.google.com', '_blank')}
                    className="mt-6 inline-flex items-center gap-2 border border-white/30 px-8 py-4 text-sm font-extrabold tracking-wider uppercase hover:bg-white hover:text-black transition-all rounded-lg">
                    Lihat Lokasi
                  </button>
                </div>
              ))}
              ...
              <div className="pt-4">
                ...
                <p className="mt-3 max-w-lg" style={{ fontSize: '16px', lineHeight: '20px', color: 'rgb(196,201,203)' }}>Kami dengan hormat menganjurkan para tamu kami untuk mengenakan warna-warna ini untuk hari istimewa kami.</p>
                <div className="flex gap-6 mt-8">
                  {['#1a1a2e', '#16213e', '#0f3460', '#533483', '#e94560', '#0a0a0a'].map((color, i) => (
                    <div key={i} className="flex flex-col items-center gap-2">
                      <div className="w-12 h-12" style={{ background: color }} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Live Wedding — separate card */}
            <div className="mt-8 text-left p-10 bg-white/15 backdrop-blur-sm rounded-lg">
              ...
              <p className="mt-4 max-w-md" style={{ fontSize: '16px', lineHeight: '20px', color: 'rgb(196,201,203)' }}>Moment bahagia prosesi pernikahan akan kami tayangkan secara virtual melalui platform berikut ini.</p>
              <button onClick={() => window.open('https://youtube.com', '_blank')}
                className="mt-8 inline-flex items-center gap-2 border border-white/30 px-10 py-4 text-base font-extrabold tracking-wider uppercase hover:bg-white hover:text-black transition-all rounded-lg">
                Live Streaming
              </button>
            </div>
          </div>
      ...
      <section id="rsvp-section" ref={setSectionRef} className="px-6 py-32">
        <div className="max-w-lg mx-auto text-left">
          ...
          {rsvpSubmitted ? (
            <div className="mt-12 p-12 bg-white/[0.04] backdrop-blur-sm rounded-lg">
            ...
            <div className="mt-12 bg-white/[0.04] backdrop-blur-sm p-10 text-left space-y-6 rounded-lg">
              ...
              {rsvpStatus === 'attending' && (
                <div>
                  <label className="text-base font-bold opacity-50">Jumlah Tamu</label>
      ...
      <section ref={setSectionRef} className="px-6 py-24">
        <div className="max-w-lg mx-auto">
          {wishSubmitted ? (
            ...
            <div className="mb-12 bg-white/[0.03] p-8 text-left space-y-4 rounded-lg">
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto">
            {wishes.slice(0, 20).map((w) => (
              <div key={w.id} className="p-6 text-left text-base bg-white/[0.03] rounded-lg">
                <div className="flex items-start justify-between gap-3">
                  <p className="font-extrabold text-lg">{w.name}</p>
                ...
                <p className="mt-2 text-base opacity-50 font-bold">{w.message}</p>
                {w.reply && (
                  <div className="mt-3 ml-3 pl-4 border-l-2 border-white/10">
                    <p className="text-sm flex items-center gap-2 opacity-30 font-bold"><CornerDownLeft className="h-4 w-4" /> Balasan</p>
                    <p className="text-base opacity-50 mt-1 font-bold">{w.reply}</p>
                  </div>
                )}
              </div>
            ))}
            {wishes.length === 0 && <p className="text-left text-lg opacity-20 py-12 font-bold">Belum ada ucapan. Jadilah yang pertama!</p>}
          </div>
        </div>
      </section>
          ...
          {giftTab === 'envelope' ? (
            <div className="mt-10 space-y-4">
              {[
                ...
                <div key={i} className="bg-white/[0.03] p-6 text-left flex items-center justify-between rounded-lg">
                  <div>
                    <p className="text-sm font-extrabold uppercase tracking-wider opacity-30">{item.bank}</p>
                    <p className="font-bold text-xl mt-1">{item.number}</p>
            ...
            <div className="mt-10 bg-white/[0.03] p-12 rounded-lg">
      ...
      {guestName && (
        <section ref={setSectionRef} className="px-6 py-24 text-left">
          <div className="max-w-md mx-auto bg-white/[0.03] p-10 backdrop-blur-sm rounded-lg">
            <p className="text-lg tracking-[0.3em] uppercase opacity-40 font-bold">QR Check-In</p>
            <p className="text-lg opacity-50 mt-2 font-bold">Halo,</p>
            <p className="text-3xl font-extrabold mt-2">{guestName}</p>
            ...
            <p className="text-base opacity-30">Tunjukkan QR ini saat check-in di venue</p>
          </div>
        </section>
      )}
      ...
      <section ref={setSectionRef} className="px-6 py-40 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg mx-auto">
          ...
          <p className="mt-6 text-xl opacity-40 font-bold">Atas kehadiran dan doa restunya</p>
          ...
          <p className="mt-4 text-lg opacity-30 font-bold">Kami yang berbahagia,</p>
      ...
      <footer className="px-6 py-16 text-left">
        ...
        <p className="text-lg opacity-40 font-bold">Terima kasih atas doa & restu</p>
        <p className="text-base opacity-20 mt-3 font-bold">&copy; WeddingInvite · {data.title || `${data.groomNickname || data.groomName} & ${data.brideNickname || data.brideName}`}</p>
      </footer>
    </div>
  )
}
